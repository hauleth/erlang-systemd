...
groups() ->
    [
        {auto_install, [shuffle], [
            non_standard_handler,
...
custom_parent(_Config) ->
    ...
    ok = systemd_kmsg_formatter:check_config(#{
        parent => logger_formatter,
        depth => 2
    }),
    {error, _} = systemd_kmsg_formatter:check_config(#{
        parent => logger_formatter,
        unknown => 10
    }),
...
auto_install(Config) ->
    ...
    ok = systemd_kmsg_formatter:auto_install(),
    {ok, #{formatter := {systemd_kmsg_formatter, #{parent := Parent}}}} = logger:get_handler_config(
        Type
    ),
